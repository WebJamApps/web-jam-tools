...
export function renderDarkHtml(result: BookGigResult): string {
  ...
  if (result.batchDispatch) {
    ...
    const skippedRows = b.skipped.map((s, idx) => `
      <tr data-venue-id="${escapeHtml(s.venueId)}">
        <td class="num-col">${idx + 1}</td>
...
<head>
  ...
  <style>
    ...
    * {
    ...
    table.candidate-table th.sortable-th {
      cursor: pointer;
      user-select: none;
      transition: background-color 0.15s ease, color 0.15s ease;
    }

    table.candidate-table th.sortable-th:hover {
...
${
    JSON.stringify({
      ...
      batchDispatch: result.batchDispatch,
...
/**
 * Extract structured run data from an existing Dark Mode HTML artifact.
 * Reads the embedded JSON script tag if present, falling back to DOM regex parsing.
 */
export function extractRunDataFromHtml(htmlContent: string): {
  ...
  const candidateRowRegex = /<tr\s+data-venue-id=["']([^"']+)["'][^>]*>([\s\S]*?)<\/tr>/gi;
  let match: RegExpExecArray | null;
  while ((match = candidateRowRegex.exec(htmlContent)) !== null) {
    const venueId = match[1];
    const rowHtml = match[2];
    const venueNameMatch = rowHtml.match(
      /<td class="venue-name">\s*<strong>(.*?)<\/strong>\s*<\/td>/i,
    );
    if (venueNameMatch) {
      const name = venueNameMatch[1].trim();
      candidates.push({
        _id: venueId,
        name,
      });
  ...
  const pitchCardRegex =
    /<section\s+class="pitch-card"[^>]*data-venue-id=["']([^"']+)["'][^>]*>([\s\S]*?)<\/section>/gi;
  while ((match = pitchCardRegex.exec(htmlContent)) !== null) {
    const venueId = match[1];
    const cardHtml = match[2];
    const venueMatch = cardHtml.match(/<h3 class="pitch-venue">(.*?)<\/h3>/i);
    const toMatch = cardHtml.match(/<a\s+href="mailto:([^"]+)"/i);
    const subjectMatch = cardHtml.match(/<strong>Subject:<\/strong>\s*<span>(.*?)<\/span>/i);
    const bodyMatch = cardHtml.match(/<pre class="pitch-body"[^>]*>([\s\S]*?)<\/pre>/i);
  ...
  const dispatchMatch = htmlContent.match(
    /<p><strong>Dispatched:<\/strong>\s*(\d+)\s*of\s*(\d+)\s*venue pitch emails sent\.<\/p>/i,
  );
  if (dispatchMatch) {
    batchDispatch = {
  ...
  if (candidates.length > 0 || pitches.length > 0 || batchDispatch) {
    return { candidates, pitches, batchDispatch };
  }
  return null;
}
