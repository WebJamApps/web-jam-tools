...
// Evaluates to ~7.7KB currently; 8,500 bytes is the hard upper bound limit.
Deno.test("real memory directory index generation budget check (<= 8500 bytes)", async () => {
  ...
  assert(
    byteCount <= 8500,
    `Memory index size (${byteCount} bytes) exceeds budget of 8,500 bytes`,
  );
});
