...
2. Format feedback with clear section headers and severity icons:
   ...
   - **Suggestions** (`### 🟡 Suggestions`): Specific code references or line numbers where concrete code-quality or design improvements are suggested. Prefix each suggestion line with 🟡. If none, render `### Suggestions` with `✅ None`.
     - **Exclusion of Process & Git Mechanics Trivia**: This section is strictly for code-relevant improvement suggestions tied directly to the diff under review. It **never** carries process/mechanics trivia, speculative workflow commentary, or git/semver heads-ups that are not defects in the PR itself. Specifically, **never post cross-PR version-bump collision warnings** (e.g. noting that another open PR shares the same semver target and might merge first) or git workflow lectures — those describe another PR's state, not a defect in the diff under review. A gratuitous double version bump made *within the PR under review* is different: Step 2 item 5 evaluates that PR's own commits and reports such a bump as a Suggestion, so it belongs in this section as a real, permitted finding. A version that fails to strictly exceed `origin/dev` at review time is the other case Step 2 item 5 covers, and that one is caught deterministically as a Must Fix, not a Suggestion.
