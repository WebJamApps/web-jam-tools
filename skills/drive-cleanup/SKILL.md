...
Drive may still contain snapshot copies of `gemma-tasks.txt` / `claude-opus-tasks.txt` / `SHARED.md` / `GEMMA.md` for visibility from a browser, but those are NOT authoritative — only the Dropbox copies are. After any bridge action, refresh the relevant Drive snapshot from Dropbox via rclone so a Drive glance shows the current state.
...
- **Multiple files with the same name** — flag any duplicates. The canonical files at root should each appear EXACTLY ONCE:
  - `claude-opus-tasks.txt` (id `1Rz5yi81zy5ohwirUJsliF1KONIXUIQQL`) — Opus queue **snapshot** (mirror of Dropbox; NOT authoritative)
  - `gemma-tasks.txt` (id `1PiobgF2vPhimDtTpQnjkWSaNQ6zaYI-g`) — Coordinator queue **snapshot** (mirror of Dropbox; NOT authoritative)
...
- **Allowed non-queue files at root (whitelist — DO NOT flag):**
  - `SHARED.md` (id `1X48-YCTaYScEIEJNaD4__imsMfWwMoRr`) — Drive snapshot of the Dropbox authoritative copy.
...
|---|---|---|---|
| 1 | Sonnet bridge file pending | for-gemma-cavendish-cleanup.txt (id X) | Append to Dropbox gemma-tasks.txt; verify; rename Drive original to processed-2026-05-21-cavendish-cleanup.txt; refresh Drive snapshot via rclone |
...
5. **Refresh the Drive snapshot**: push the updated local file back to Drive via rclone — `rclone copy /home/joshua/Dropbox/web-jam-llms/<queue>.txt gdrive: --update`. This keeps any browser-side glance current. Do this once per queue, after all bridges for that queue have completed.
6. **Append to `bridge-log.md`** at `/home/joshua/Dropbox/web-jam-llms/bridge-log.md`: timestamp (UTC), source filename, dest path, bytes appended, status (ok | failed-verify).
...
- **Never delete a canonical queue** — neither the Dropbox originals (`gemma-tasks.txt`, `claude-opus-tasks.txt`) nor their Drive snapshots, nor `claude-sonnet-tasks.txt` on Drive.
- **Never delete `SHARED.md` or `GEMMA.md`** — neither Dropbox originals nor Drive snapshots.
