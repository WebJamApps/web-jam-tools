...
16. **Set the native Priority field, then verify it by reading it back — a write response is never confirmation.** Set via GitHub MCP `issue_write` → `issue_fields` (or `scripts/create-issue.ts --priority <Level>`). `gh` CLI cannot set a native field, and `gh api graphql` cannot even read one under the pinned PAT (`INSUFFICIENT_SCOPES ... requires ... read:project`) — that error means the wrong tool was used, not that Priority is unverifiable. The GitHub MCP server authenticates separately and reads/writes the field without difficulty.
    ...
    - **Read back, don't trust the write.** After setting Priority, read back every touched issue's Priority via `mcp__claude_ai_GitHub_MCP__list_issues` (`fields: ["number","title","field_values"]`) for the batch, or `mcp__claude_ai_GitHub_MCP__issue_read` for one issue, and compare each against the plan table's `Priority` column.
