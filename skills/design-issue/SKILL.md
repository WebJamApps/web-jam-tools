...
6. **GATE 1 — stop.** Render the design to a standalone HTML file via `scripts/render_design_doc.ts` (layout rules applied) and verify it via headless screenshot. The headless screenshot only proves the layout isn't broken to the agent — it does not put the document in front of Josh. Once verified, open it for him with the literal command:
   ...
   ```sh
   google-chrome "file:///absolute/path/to/<topic>-design-<YYYY-MM-DD>.html" &
   ```

   Run in the background (trailing `&`) so it does not block the session — it opens a tab in Josh's existing Chrome session and returns immediately. A correct result is Chrome opening or focusing a tab showing the rendered design document. Only then present the design and wait for Josh's explicit approval. No writes to GitHub before Gate 1.
