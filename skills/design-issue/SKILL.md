...
- **Gate 1 also runs `deno task design:verify-citations` before it renders or opens anything.** This reads every issue the document cites and fails the run on a title that has drifted from what's quoted, or on a citation to a now-closed issue that the document doesn't acknowledge as closed. Both checks are required — a passing `design:lint-doc` run alone is not enough.
