...
1. **Establish the theme and milestone.** `<Theme>` matches the GitHub milestone — read the live milestone list, never assume a fixed set. **Resume rather than restart:** Resolving the canonical design document is a precondition of the run itself rather than of creating one, so every run — including one that creates no document at all — looks in the theme folder for an existing `<topic>-design-*.md` and continues from its decision record; a run that wants a fresh document says so. Otherwise create the design document at `~/Dropbox/web-jam-llms/<Theme>/<topic>-design-<YYYY-MM-DD>.md`, creating the theme folder if the milestone has none yet.
   - **Automatic Feature Matching & Canonical Document Discovery:** On every design run — Epic, Feature, Task or Bug alike, and whether or not a new document will ever be created — run `deno task design:candidates --epic <citation>` (or `deno task design:match-design "<topic>"`) to automatically scan `~/Dropbox/web-jam-llms/<Theme>/` (and adjacent theme directories) for pre-existing canonical design documents (`<topic>-design-*.md` or `<topic>-skill-design-*.md`), as step zero above requires.
...
1. **Automatic Discovery & Resolution:** As the run's first act — the step zero of Phase 1, reached before any decision is put to Josh and whether or not a new file will ever be created — run `deno task design:candidates --epic <citation>` or `deno task design:match-design "<topic>"` to locate the pre-existing canonical design document (`<topic>-design-*.md` or `<topic>-skill-design-*.md`) across the theme directory (and adjacent theme directories). The command resolves to the canonical document rather than spawning a new standalone file. A resolved document is then read end to end, decisions-record appendix included, and what it already settles is stated in the conversation before the first decision is put to Josh; the command prints that decisions record alongside the path, so an unread decisions record is visible in the run rather than silent, but printing it is not reading it and the document itself is still opened.
...
16. **Set the native Priority field, then verify it by reading it back — a write response is never confirmation.** Set via GitHub MCP `issue_write` → `issue_fields` (or `scripts/create-issue.ts --priority <Level>`). `gh issue create` / `gh issue edit` cannot set a native field; `scripts/create-issue.ts` sets it with a `gh api graphql` `updateIssueFieldValue` mutation, then re-reads the issue and exits with an error on a Priority mismatch — so an issue filed by a successful `create-issue.ts --priority` run is already verified and needs no second read-back. **Reading the field:** a `gh api graphql` query for it fails under the pinned `GH_TOKEN` (`INSUFFICIENT_SCOPES ... requires ... read:project`) — that error means the wrong tool was used, not that Priority is unverifiable. The REST read `gh api repos/<owner>/<repo>/issues/<n> --jq '.issue_field_values'` works under that same token on both surfaces, as does the GitHub MCP read below. (On Claude Code the claude.ai GitHub connector authenticates separately from the pinned token; on agy the `github` MCP server runs on the pinned `GH_TOKEN` itself.)
    - **Scope: every issue the run touched, not only the ones it created.** This includes issues re-parented into an epic or otherwise claimed for a plan row without creating a new issue — a plan row that points at an existing issue still carries a Priority from the plan table, and that Priority must be applied and verified exactly as a newly filed issue's is.
    - **Read back, don't trust the write.** After setting Priority, read back every touched issue's Priority — except one filed by a successful `create-issue.ts --priority` run — via `mcp__*__list_issues` (`fields: ["number","title","field_values"]`; on Claude Code `mcp__claude_ai_GitHub_MCP__list_issues`) for the batch, `mcp__*__issue_read` (on Claude Code `mcp__claude_ai_GitHub_MCP__issue_read`) for one issue, or the REST read above, and compare each against the plan table's `Priority` column.
    - **Missing or mismatched → set, then re-read.** Where the plan carries a Priority and the read-back shows it missing or different, set it via `issue_write` → `issue_fields` and re-read to confirm — the re-read, not the write response, is what closes the row.
    - **Report every unresolved row to Josh, explicitly.** Cite the issue as `repo#number "title"` with the expected and actual values. The run does not report the filing as complete while any row's Priority is unverified.
    - **A failed read-back is reported, not asked about.** If the read-back itself cannot be performed (the MCP call errors, times out, or returns unparseable data), say so plainly, name exactly which issue(s) could not be verified, and do not report success — never convert the failure into a question asking Josh to go check in a browser.
