...
**Every directive is designed before Gate 1, never narrowed and parked at Gate 2.** Every directive line in the target issue's body, and in the body of any issue the run will close or remove `Needs Design` from, is designed in full and proved before Gate 1. Narrowing a directive to a smaller reading is a decision: it goes to Josh by name before Gate 1, following the Decision-Readiness rule below, and is never written only into a Gate 2 `Needs Design` removal's "not resolved" part, where an approval of the plan would approve the cut without Josh ever seeing it (web-jam-tools#1131 "skills/design-issue: a target issue's requirement can be narrowed and parked as not resolved at Gate 2 instead of designed before Gate 1").

0. **STEP ZERO — resolve the canonical design document, and read it, before any decision is put to Josh.** This is a precondition of the *run*, not of creating a file: it happens at the start of every design run (Epic, Feature, Task or Bug alike), including a run that will never write a document. Resolve the topic with `deno task design:match-design "<topic>"` (or `deno task design:candidates --epic <citation>`) to automatically scan `~/Dropbox/web-jam-llms/<Theme>/` (and adjacent theme directories) for pre-existing canonical design documents (`<topic>-design-*.md` or `<topic>-skill-design-*.md`), and act on exactly one of three outcomes:
   - **A canonical document is resolved (exit 0).** Read it end to end — decisions-record appendix included — and state in the conversation what it already settles, before any decision is put to Josh. The run continues as a **Major Revision** to that document, updating the specification in-place rather than spawning a new standalone file: it adds or updates the document's structured revision table with one new row, reconciles the architecture, decisions and `## Both surfaces` sections in place so the document reads cohesively as though it had always been the design, and refuses outright to create a second, parallel document (`*-phase-2-design-*.md` or similar) for a topic that already has one. Gate 1 (`runGate1`) actively enforces this check in code and refuses to render or open any document that duplicates a pre-existing canonical topic. A decision the document already records is never re-asked as though it were open; where the run believes a recorded decision should change, it names that decision and says why, and Josh rules on that.
   - **No canonical document exists (exit 1).** Say so, and proceed to create a new design document exactly as step 1 describes. Nothing else changes for a genuinely new topic.
   - **The check cannot tell which of the two applies (exit 2).** The design-documents root or theme folder is missing, Dropbox is not mounted, a directory is unreadable, the topic cannot be resolved, or the resolved document cannot be read. **The run REFUSES and stops**, reporting the path named in the refusal. It never continues as though no document existed — a silent "no document found" and a silent "could not look" are indistinguishable to the agent, and both let a run redesign a feature that is already designed, so the check fails closed. (The design-documents root is `~/Dropbox/web-jam-llms`, overridable for a scoped run via `DESIGN_DOCS_ROOT` or `--dropbox-dir`.)
1. **Establish the theme and milestone.** `<Theme>` matches the GitHub milestone — read the live milestone list, never assume a fixed set. **Resume rather than restart:** Resolving the canonical design document is a precondition of the run itself rather than of creating one, so every run — including one that creates no document at all — looks in the theme folder for an existing `<topic>-design-*.md` and continues from its decision record; a run that wants a fresh document says so. Otherwise create the design document at `~/Dropbox/web-jam-llms/<Theme>/<topic>-design-<YYYY-MM-DD>.md`, creating the theme folder if the milestone has none yet.
...
7. **GATE 1 — stop.** Present the design with `deno task design:gate1 <doc.md>` and no other way: it checks the document, renders it to a standalone HTML file, verifies the render with a headless screenshot, opens it for Josh in his own browser session in the background on his active display, and opens the run's **Gate 1 record** for that document (see "The Gate 1 record" below). Rendering, screenshotting and opening the document by hand skips the record, so the run cannot reach Gate 2 afterwards. Present the design and wait for his explicit approval. No writes to GitHub before Gate 1.

### What counts as Gate 1 approval

Gate 1 passes only on a reply to the Gate 1 presentation that approves the design. A question, a correction, a new or changed requirement, and an instruction about a later step — which epic the Gate 2 plan should use, how issues should be filed, what to do once the plan is approved — are not approval, however they are phrased. The run acts on such a reply: it answers the question, or applies the change to the document and presents Gate 1 again with `deno task design:gate1`, and then asks whether the design is approved. A reply that mixes approval with anything else is answered first and approval asked for again. When the run cannot tell whether a reply approves the design, it asks; it never infers approval from an instruction about what happens after it.

Once Josh approves, the run records it with `deno task design:gate1-approve <doc.md>`, passing his approving reply word for word. That quote is what the Gate 2 presentation opens with, so a reply the run misread as approval is the first thing Josh sees at Gate 2, not something he has to notice later.

### The Gate 1 record

The run's position at Gate 1 lives in a small record on disk rather than only in the conversation, because a conversation can be compacted: the summary an agent writes itself when its context fills can drop "Gate 1 is waiting on Josh", and the run then resumes believing it is past a gate it never passed. The record survives that.

- **Where it lives.** One JSON file per design document under `~/.claude/state/design-gate1/`, named from a hash of the document's absolute path, beside the Gate 2 approval token, and overridable for tests. Both surfaces run the same `deno task` entries, so both read and write the same file.
- **What it holds.** The document's absolute path; the document's content fingerprint (SHA-256) when it was presented; when it was presented; and, once approved, when, the approving reply verbatim, and the fingerprint the approval covers.
- **`deno task design:gate1 <doc.md>`** writes the record as open each time it presents the document, clearing any earlier approval — re-presenting a revised design always needs a fresh approval.
- **`deno task design:gate1-approve <doc.md> --reply <text>`** (or `--reply-file <path>`) marks it approved. It **refuses** when the document has no record (Gate 1 was never presented through `design:gate1`), when the document's fingerprint no longer matches the one presented (it changed after Josh saw it, so it is presented again first), and when the reply is empty. When the record or the document cannot be read, it **refuses**.
- **`deno task design:gate1-status <doc.md>`** prints where Gate 1 stands for that document — not presented, open, approved (with the quoted reply), or changed since approval. After a compaction or a resume, the run reads this rather than trusting its own summary, and acts on what it says: an open Gate 1 is asked about again, never assumed passed.
- **`deno task design:lint-plan <plan.md> --design-doc <doc.md>`** checks the record before Gate 2 can be presented. It **passes** when the record shows an approval of Gate 1 and the document's fingerprint still matches the approved one, printing `Gate 1 approval recorded <date>: "<reply>"`. It **refuses** when there is no record, when Gate 1 is still open, when the document changed after approval, or when `--design-doc` is omitted. When the record or the document cannot be read, it **refuses**.

The agent still judges whether Josh's reply approves the design; nothing parses his wording for approval phrases, which he has ruled out. What the record adds is that the judgment is made once, explicitly, against a quoted reply that Gate 2 puts back in front of him, and that no summary can lose it.

### The Measure-First Run

Some topics cannot be designed yet. The design's mechanism rests on a premise that no amount of reading settles — an inventory nobody has taken, a probe nobody has run, a behaviour observable only on a surface this run cannot reach — and obtaining it is work in its own right rather than a lookup. A run in that position is a **measure-first run**.

**How the run recognises itself.** Phase 1 step 4 requires every load-bearing premise to be proven before Gate 1, and a premise fails that test in two different ways with two different remedies. A premise that is merely unchecked gets checked, and the run continues. A premise that cannot be checked from what exists converts the run at that moment. The test is whether obtaining the proof is a lookup or a piece of work: if it needs doing rather than reading, the run converts.

**What the run does.** The run carries out the measurement itself, inside the same run, before proposing anything. It delegates the measurement to the cheapest tier that can reliably do it — an inventory or an audit typically goes to a Sonnet subagent, with Opus checking the judgment calls the measurement turns up (classifications, borderline calls) rather than doing the mechanical scan itself. The run proposes no issues and presents no plan table while the measurement is outstanding — Gate 2 does not fire on a measurement that has not yet reported. Once the measurement reports, the run revises the canonical design document (or writes it, if none existed) from what the measurement found, and presents that revised document at **GATE 1** exactly as any other design does. Gate 1 approved, the run proceeds to Phase 2 and **GATE 2** as normal — the measurement never becomes its own plan-table row, because it already happened before Gate 1 opened.

**This is not dispatching build work.** The absolute standing rule bars the skill from spawning a build agent, handing work to a lane, starting a worktree, or running `/work-issue` — it governs implementation work the run would otherwise hand off to be built and merged. Delegating the measurement to a cheap-tier subagent is different in kind: the measurement is itself part of the design run's own Phase 1 fact-finding (Phase 1 step 4, "verify facts rather than asserting them"), carried out at a larger scale, not a deliverable being built and shipped. The two rules do not conflict because they govern different things: the never-dispatches rule bars handing off *building* what the design will command; the measure-first run only ever hands off *finding out* what is true, and that finding-out happens before Gate 1, inside the same run, with nothing shipped and no issue filed as a side effect of it.

**When the measurement cannot be performed inside the run.** Some measurements genuinely cannot be done by any agent in this run — a live demonstration on hardware only Josh has, a UI probe only his own logged-in session can trigger, a credential no agent holds. Only then is the measurement filed as an issue, and only when Josh asks for it to be — the run never files it on its own initiative. A measurement that is a manual step only Josh can perform still gets a numbered runbook exactly as any other manual step does (see Manual Steps & Verification Pairs), and is handed to him directly in chat rather than filed, unless he asks for it to become an issue.

**Where the run's decisions live.** A measure-first run still rules decisions, and the settled ones are settled permanently — a remit, a policy, a scope boundary, anything measurement cannot overturn. Those are written into the canonical design document as Josh gives them, exactly as Phase 1 step 3 requires, before the measurement runs.

**The gates.** Gate 1 does not fire until the measurement has reported and the document has been revised from its findings — a measure-first run presents nothing at Gate 1 while the measurement is still outstanding, because there is no design yet to approve. Once the measurement reports and the document is revised, Gate 1 fires against that revised document like any other run's. Gate 2 fires afterward, unchanged, against the resulting issue plan — which never contains the measurement as a row, because the measurement already happened inside this run, before Gate 1.
...
12. **GATE 2 — stop.** Run `deno task design:lint-plan <plan.md> --design-doc <doc.md>` first; the plan is not presented unless it passes, and the presentation opens with the `Gate 1 approval recorded <date>: "<reply>"` line it prints. Present the plan and wait for Josh's explicit issue plan approval. No creating, editing, or labeling issues before Gate 2. Gate 2 approval of the plan authorizes the `Needs Design` label removals listed in the plan. Nothing is filed to GitHub until Gate 2 passes.
   ...
   **Write the issue-approval token upon Gate 2 approval:** The moment Josh explicitly approves the plan table, write the issue-approval token for the planned repo and exact titles (via `scripts/write_issue_approval_token.ts`) so subsequent `mcp__*__issue_write` and `mcp__*__sub_issue_write` calls pass `hooks/require-approval-token-on-issue-write.sh` without repetitive authorization prompts:
   ...
   The token records the approving session's id, the target `owner/repo`, the exact approved titles, and a bounded expiry (default 4 hours) at `$HOME/.claude/state/issue-approval-tokens/<session-id>.json` (honoring `ISSUE_APPROVAL_TOKEN_PATH`).
...
19. **Report what was filed to Josh in the conversation, including the Priority verification result for every row (item 16).** The design document is never edited to record filing status — the filed issues themselves, cited by repo + number + title, are the record of what happened. The report never states filing as complete while a row's Priority is unverified.
...
A measure-first run reaches Gate 1 like any other run — only later: Gate 1 waits until the run's own measurement has reported and the document has been revised from it, since there is no design to approve until then. Gate 2 fires afterward, unchanged; it never presents the measurement as a plan-table row, because the measurement already happened inside the run, before Gate 1.
...
| present a design at Gate 1 whose mechanism rests on measurement that has not been performed | a document waiting on evidence is not a design but a plan to go and get one; the run converts to a measure-first run, performs the measurement itself, and the design is written from what that measurement reports |
| present an issue plan, or ask for issue-plan approval, in a measure-first run before the measurement has reported | Gate 2 cannot fire on a measurement that has not happened; the run performs the measurement first, revises the design from it, and only then presents Gate 1 and Gate 2 in the normal order |
| create, edit or label an issue before GATE 2 | the plan gate, likewise |
| present an issue plan, or ask for issue-plan approval, while `design:lint-plan` cannot find an approval of Gate 1 in the Gate 1 record for the unchanged design document | a gate that lives only in the conversation disappears when the conversation is compacted, and the run then walks past it believing it already passed |
| take a question, a correction, a new requirement, or an instruction about a later step as Gate 1 approval | an instruction about what to do at Gate 2 is not approval of the design; the run acts on it and asks again |
| present Gate 1 any way other than `deno task design:gate1` | rendering and opening the document by hand skips the Gate 1 record, and the run can then never show Gate 1 was passed |
...
| continue a design run when the canonical-document check cannot tell whether a document exists — a missing design-documents root or theme folder, an unmounted Dropbox, an unreadable directory, an unresolvable topic, or a document that cannot be read | a silent "no document found" and a silent "could not look" are indistinguishable to the agent, and both let a run redesign an already-designed feature; the check fails closed, exiting `2` and naming the path it could not read, and the run stops rather than proceeding |
...
| narrow a directive of the target issue, or of any issue the run closes or removes `Needs Design` from, and record the cut only as a Gate 2 "not resolved" part | a requirement cut down at Gate 2 is approved without Josh seeing it; every directive is designed and proved before Gate 1, and narrowing one goes to Josh by name first (web-jam-tools#1131 "skills/design-issue: a target issue's requirement can be narrowed and parked as not resolved at Gate 2 instead of designed before Gate 1") |
...
- **Every literal command is proven runnable before the runbook ships:** Writing a command out literally is not the same as knowing it runs. Each literal command in a runbook is either executed during the authoring run, or — the normal case for a manual step the author cannot perform — has its invocation form **verified against** the tool's own `--help` (or equivalent usage) output, confirming the flags, argument order and prompt-passing form the tool actually accepts. Which of the two was done, and for which commands, is recorded in the authoring issue. This governs how the AUTHOR validates a command before writing it down: it never licenses adding a verification step to the walkthrough itself (see **No conflating internal verification assertions with the human walkthrough script** below), and the `--help` check happens at authoring time, never as a side-quest during Josh's walkthrough (see **Dashboard drift & walkthrough safety**).
...
- **Every premise row also carries a `Proved` date, and it has to be current.** The `## Load-bearing premises` table carries a `Proved` column — an ISO `YYYY-MM-DD` date recording when that row's proof was last actually run, matched by column name rather than position. `deno task design:lint-doc` fails a row whose `Proved` date is missing, malformed, or predates the run presenting the document: a premise proved on an earlier day cannot be asserted as proven today. Same-day is the pass condition.
- **A design document carries no GitHub issue/PR citation outside the `## Revision History` table or a verbatim quote.** A design document records how the system works and what was decided — never GitHub issue/PR history: not a bug's status, not "(closed)"/"(since closed)", not a review-round or "fixed by" narrative. Issue and PR history already lives in GitHub and git, and a second copy of it in a design document only goes stale. The two places a citation (`repo#N`, `owner/repo#N`, bare `#N`, or a `github.com/.../issues|pull/N` URL) may still appear are the `## Revision History` table, which is a version-to-issue map rather than a status narrative, and a verbatim quote of Josh's own words, which is never paraphrased to remove what he typed. `deno task design:lint-doc` fails a document carrying a citation anywhere else, naming the line and the citation.
- **Gate 1 also runs `deno task design:verify-citations` before it renders or opens anything.** This reads every issue or pull request the document cites and fails the run on a title that has drifted from what's quoted. It does NOT require a closed citation to be acknowledged as closed — a citation living anywhere that acknowledgement would apply is already a `design:lint-doc` failure under the rule above, so there is no location left where an unacknowledged closed citation could legitimately sit. Both checks are required — a passing `design:lint-doc` run alone is not enough.
