...
- **EMAIL SEND CARVE-OUT — `/book-gig` gated batch dispatch only (D-47):** The single named
  ...
  approval of the rendered draft copy itself). Gate 1 approval of the venue set never authorizes
  dispatch on its own, and neither gate may ever be inferred from the other or from any other
  statement Josh makes — see `skills/book-gig/SKILL.md` for the full gate mechanics and fail-closed
...
- **Third-party contact carve-out (same one as above, D-47):** The `/book-gig` gated batch
