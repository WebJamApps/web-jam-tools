...
| 4 | **Sonnet** *(Claude Code subagent)* | Laptop | Coder (Claude-side) | Coding work that needs a Claude-side capability Flash lacks. **No longer the default coder** — Flash High now outranks it on contained implementation work and bills a separate budget, so route here only for a named Claude-specific reason. | *(issued via GitHub label)* |
...
| 6 | **Flash** *(Gemini Flash High / Med via agy/Antigravity)* | Laptop | Primary Coder | Full-stack coding (FE, BE, APIs, tooling) across all 8 repos + general agy lane, and now **the default implementation tier ahead of Sonnet** — see "Tier order" below. Invoked via `/work-issue` or interactive `agy`. | *(issued via GitHub label — `agy-tasks.txt` retired, web-jam-tools#249)* |
...
- **Tier order, weakest to strongest: Haiku → Flash Med → Sonnet → Flash High → Opus.** Flash High sits **above** Sonnet as of 2026-09-05: on contamination-resistant long-horizon coding (DeepSWE v1.1) Gemini 3.8 Flash at high effort scores 73.7% against Sonnet 5's 54%, effectively matching Opus 5's 74%, at under $2.40 a task against Opus's $11-plus. Opus keeps the top slot on the strength of the one place that parity does not hold — abstract, multi-step unguided agent work (Terminal-Bench 4.0), which is what open-ended design and review actually are. This ordering governs both routing and who may review whose PR (`skills/pr-review/SKILL.md`).
- **Always delegate to the cheapest model that can do the job.** Haiku for mechanical one-offs & doc cleanups, Flash High for full-stack coding & interactive work (the primary Flash lane, and now the default implementation tier for contained *and* multi-file coding), Flash Med only for genuinely trivial edits (a one-line change, a single-field data or typo fix, a link update), Sonnet only when a task needs a Claude-side capability Flash lacks — name the reason — and Opus for judgment and design only. Spending Opus or Sonnet tokens on work Flash High now does better wastes the constrained Anthropic budget twice over.
...
- **Adaptive Escalation Rule (Flash High → Subagent Boost):** Antigravity (agy) sessions should use **`Flash High`** as the standard tier for full-stack coding and interactive work; reserve `Flash Med` for genuinely trivial edits only. If a subtask encounters complex multi-file refactoring, difficult type/lint errors, or intricate REAPER routing/composition, the session automatically dispatches a subagent with `Model: "inherit"` (`invoke_subagent` inheriting `gemini-3.8-flash-high`) to solve that specific block and report back. For contained coding tasks delegated down to Flash Medium, use `invoke_subagent` with `Model: "flash"` (which runs cleanly under `gemini-3.8-flash-tiered`).
...
| Ordinary contained coding task | **Flash High** (agy/Antigravity) via `/work-issue` |
| Multi-file refactor or complex backend coding | **Flash High** — it outranks Sonnet on this work now |
| Coding that needs a Claude-side capability Flash lacks | **Sonnet** subagent (via Opus) — name the capability |
