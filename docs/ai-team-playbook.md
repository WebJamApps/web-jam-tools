...
- **Adaptive Escalation Rule (Flash High → Subagent Boost):** Antigravity (agy) sessions should use **`Flash High`** as the standard tier for full-stack coding and interactive work; reserve `Flash Med` for genuinely trivial edits only. If a subtask encounters complex multi-file refactoring, difficult type/lint errors, or intricate REAPER routing/composition, the session automatically dispatches a subagent with `Model: "inherit"` (`invoke_subagent` inheriting `gemini-3.8-flash-high`) to solve that specific block and report back. When delegating subagent tasks via `invoke_subagent`, `Model: "flash"` runs as `gemini-3.8-flash-tiered` and passes the model guard.
