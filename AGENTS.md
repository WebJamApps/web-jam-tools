...
9. **Skill Renames & Stale Symlink Pruning:** `skills/issue-design/` is renamed to `skills/design-issue/` and `skills/draft-issue/` is renamed to `skills/file-issue/`. `scripts/install-skills.sh` includes an automatic prune path to remove dangling symlinks pointing to deleted skill directories in both `~/.claude/skills` and `~/.gemini/config/plugins/webjam-tasks/skills` while leaving `*.bak-*` directories untouched.
