...
20. **Gate 1 Candidate Filtering & Approval Set Integrity:**
    ...
    - **No Silent Approval Overwrites:** Because backend batch approval endpoints upsert on batchId/weekend, recording Gate 1 approval must refuse outright when an approval record already exists for the same batchId/weekend with a *different* venue set. This prevents a subsequent wider or narrower run from silently clobbering a vetted prior approval; a re-run with the identical venue set is a harmless no-op.
