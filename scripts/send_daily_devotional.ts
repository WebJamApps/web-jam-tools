...
type OauthKeys = { installed?: { client_id: string; client_secret: string }; web?: { client_id: string; client_secret: string } };
...
function todayInEastern(): { year: string; month: string; day: string; humanDate: string } {
  ...
  const monthName = new Intl.DateTimeFormat("en-US", { timeZone: "America/New_York", month: "long" }).format(new Date());
...
type GodPause = { url: string; scriptureRef: string; verseText: string; devotion: string; prayer: string; author: string };
...
async function fetchGodPause(today: ReturnType<typeof todayInEastern>): Promise<GodPause | null> {
  ...
  const todayPrefix = `https://www.luthersem.edu/godpause/${today.year}/${today.month}/${today.day}/`;
  const match = monthHtml.match(new RegExp(`href="(${todayPrefix.replace(/[.]/g, "\\.")}[0-9]+/)"`));
...
function buildRawMessage(to: string, subject: string, body: string): string {
  ...
  return Buffer.from(headers + body, "utf-8").toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
